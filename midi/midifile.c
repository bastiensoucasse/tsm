...
static void add_event(MidiFileEvent_t new_event)
{
    ...
    if (new_event->tick > new_event->track->end_tick)
        new_event->track->end_tick = new_event->tick;
}
...
int MidiFile_save(MidiFile_t midi_file, const char* filename)
{
    ...
    if ((midi_file == NULL) || (filename == NULL) || ((out = fopen(filename, "wb")) == NULL))
        return -1;
...
{
    if ((track == NULL) || ((track->last_event != NULL) && (end_tick < track->last_event->tick)))
        return -1;
...
MidiFileEvent_t MidiFileTrack_createSysexEvent(MidiFileTrack_t track, long tick, int data_length, unsigned char* data_buffer)
{
    ...
    if ((track == NULL) || (data_length < 1) || (data_buffer == NULL))
        NULL;
...
int MidiFileTrack_visitEvents(MidiFileTrack_t track, MidiFileEventVisitorCallback_t visitor_callback, void* user_data)
{
    ...
    if ((track == NULL) || (visitor_callback == NULL))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_NOTE_OFF))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_NOTE_ON))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_KEY_PRESSURE))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_CONTROL_CHANGE))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_PROGRAM_CHANGE))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_CHANNEL_PRESSURE))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_PITCH_WHEEL))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_SYSEX))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_SYSEX) || (data_length < 1) || (data_buffer == NULL))
        return -1;
    free(event->u.sysex.data_buffer);
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_META))
        return -1;
...
{
    if ((event == NULL) || (event->type != MIDI_FILE_EVENT_TYPE_META) || (data_length < 1) || (data_buffer == NULL))
        return -1;
    free(event->u.meta.data_buffer);
